...
import { useTransactions } from "@/lib/transaction-context" // Added transaction context
import { createBountyTransaction } from "@/lib/transaction-utils" // Added transaction utils
...
export function CreateBountyDialog({ children, onBountyCreated }: CreateBountyDialogProps) {
  ...
  const { addTransaction, updateTransaction } = useTransactions() // Added transaction management
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (isNaN(valueNumber) || valueNumber <= 0) {
      toast({
        title: "Invalid value",
        description: "Please enter a valid positive number for the bounty value",
        variant: "destructive",
      })
      return
    }
    ...
    try {
      console.log("[v0] Converting value:", formData.value, "to BigInt")

      // Convert DOT to Planck (1 DOT = 10^10 Planck)
      // Use Math.floor to avoid floating point precision issues
      const valueInPlanckFloat = valueNumber * Math.pow(10, 10)
      const valueInPlanck = BigInt(Math.floor(valueInPlanckFloat))

      console.log("[v0] Value in Planck:", valueInPlanck.toString())
